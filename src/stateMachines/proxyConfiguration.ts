...
export class ProxyConfiguration {
  ...
  getProxyConfig = async ({ input }: { input: ProxyConfigContext }): Promise<void> => {
    if (input.amtProfile?.proxyConfigs != null) {
      ...
      input.proxyConfigs = await this.db.proxyConfig.getByName(
        input.amtProfile.proxyConfigs[input.proxyConfigsCount].configName,
  ...
  addProxyConfigs = async ({ input }: { input: ProxyConfigContext }): Promise<any> => {
    ...
    const proxyAccessPointParameters: IPS.Models.AddProxyAccessPointParameters = {
        AccessInfo: input.proxyConfigs?.accessInfo ?? '',
    ...
    input.proxyConfigName = input.proxyConfigs?.accessInfo ?? null
  ...
  machine = setup({
    ...
    states: {
      ...
      SUCCESS: {
        entry: [assign({ statusMessage: () => 'Proxy Configured' }), 'Update Configuration Status'],
        type: 'final'
      }
    }
  })
